...
export default function InventoryItemDialog({ isOpen, onClose, item }: InventoryItemDialogProps) {
  ...
  useEffect(() => {
    async function loadProducts() {
      try {
        const response = await getProducts(1, 200); // Fetch a good number of products
  ...
  const onSubmit = async (values: InventoryFormValues) => {
    ...
    try {
      ...
      if (item) {
        ...
        response = await createInventoryItem(values as Omit<Inventory, 'id' | 'createdAt' | 'updatedAt' | 'lastUpdated'>);
      }
